angular.module("ngWizard", ["ui.bootstrap", "ngAnimate", "templates"]).directive("wizard", ["$window", "$q", function ($window, $q) {
  ...
  return {
    ...
    template: "<ng-form name='stepForm' ng-show='isActive()' class='wizard-step animate'  ng-class='animation || \"slide\"' style='overflow:auto'><ng-transclude></ng-transclude></ng-form>",
...
angular.module("src/wizardTemplate.html", []).run(["$templateCache", function ($templateCache) {
  $templateCache.put("src/wizardTemplate.html",
    '<div class="row wizard-container" style="margin-top:0;">\n' +
    '    <div class="col-md-3 col-xs-12" style="padding-right:0;padding-left:30px;">\n' +
    '        <ul class="nav nav-pills nav-stacked wizard-sidebar" style="margin-top:15px;">\n' +
    '            <li tooltip="{{getProgressPercentage() | number : 2}}%">\n' +
    "                <progressbar class=\"progress-striped\" value=\"getProgressPercentage()\" type=\"{{getProgressPercentage() == 100 ? 'success' : 'default'}}\">{{getProgressPercentage()}}%</progressbar>\n" +
    "            </li>\n" +
    ...
    '    <div class="row">\n' + '        <div class="col-xs-12 col-md-3 col-sm-3 pull-right" style="margin:10px;margin-right:20px;">\n' +
    '            <button data-toggle="modal" data-target="#mod-success" class="btn btn-primary btn-block btn-success submit animate fade-in-out" ng-hide="!isSubmittable()" ng-click="onSubmitClicked()" ng-disabled="submitting">Generate Form <i class="fa fa-circle-o-notch fa-spin" ng-show="submitting"></i></button>\n' +
    "        </div>\n" + "    </div>\n" + "</div>");
}]);